...
public class EncryptedFileTransferTests : IDisposable
{
    private readonly string _testDir;

    public EncryptedFileTransferTests()
    {
        // Create a unique test directory
        _testDir = Path.Combine(Path.GetTempPath(), "EncryptedFileTransferTests_" + Guid.NewGuid());
    ...
    {
        // Act & Assert
        Assert.Throws<ArgumentNullException>(() => new EncryptedFileTransfer(null!, "password"));
    }
    ...
    public void Constructor_ThrowsArgumentNullException_WhenPasswordIsNull()
    {
        ...
        // Act & Assert
        Assert.Throws<ArgumentNullException>(() => new EncryptedFileTransfer(mockTransfer.Object, null!));
    }
    ...
    public void Transfer_CallsDecoratedTransfer()
    {
        ...
        var encryptedTransfer = new EncryptedFileTransfer(mockTransfer.Object, "password123");

        // Act
        encryptedTransfer.Transfer(sourceFile, destFile);
    ...
    public void Transfer_ThrowsArgumentException_WhenSourcePathIsEmpty()
    {
        ...
        // Act & Assert
        var ex = Assert.Throws<ArgumentException>(() => encryptedTransfer.Transfer("", "destination.txt"));
    ...
    public void Transfer_ThrowsArgumentException_WhenDestinationPathIsEmpty()
    {
        ...
        // Act & Assert
        var ex = Assert.Throws<ArgumentException>(() => encryptedTransfer.Transfer(sourceFile, ""));
    ...
    public void Transfer_HandlesExceptions_AndCleansTempFile()
    {
        ...
        // Act & Assert
        Assert.Throws<IOException>(() => encryptedTransfer.Transfer(sourceFile, destFile));
    ...
    [Fact]
    public void Transfer_UsesEncryptionHelper()
    {
        // This test verifies that the encrypted content is different from the original
        // Arrange
        var sourceFile = Path.Combine(_testDir, "source.txt");
        var destFile = Path.Combine(_testDir, "destination.txt");
        var tempFile = Path.Combine(_testDir, "temp.txt");
        var content = "Test content for encryption verification";
        File.WriteAllText(sourceFile, content);

        // Create a mock decorated transfer that copies the temp file to a location we can examine
        var mockTransfer = new Mock<FileTransfer>();
        mockTransfer.Setup(t => t.Transfer(It.IsAny<string>(), It.Is<string>(s => s == destFile)))
            .Callback<string, string>((source, dest) => File.Copy(source, tempFile, true));
        ...
        var encryptedContent = File.ReadAllBytes(tempFile);
        var originalContent = File.ReadAllBytes(sourceFile);
        
        Assert.NotEqual(originalContent.Length, encryptedContent.Length);
